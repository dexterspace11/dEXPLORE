...
if uploaded_file:
    ...
    if len(selected) >= 2:
        ...
        if len(patterns) > 0:
            ...
            ax.set_title("PCA Projection of Clustered Patterns")
            st.pyplot(fig)

            st.subheader("Hierarchical Cluster Dendrogram")
            try:
                linkage_matrix = linkage(pdist(patterns), method='ward')
                fig, ax = plt.subplots(figsize=(8, 4))
                dendrogram(linkage_matrix, truncate_mode='level', p=10, ax=ax)
                ax.set_title("Dendrogram (Hierarchical Clustering)")
                ax.set_ylabel("Distance")
                st.pyplot(fig)
                st.markdown("The dendrogram shows how individual patterns or groups merge hierarchically. Close merges imply similar behavior, helping identify natural groupings even before clustering.")
            except Exception as e:
                st.warning(f"Dendrogram generation failed: {e}")
            ...
            if st.button("Export Analysis to Excel"):
                ...
                for r in dataframe_to_rows(df_centroids, index=False, header=True):
                    ws4.append(r)
                # Sheet: Raw Data Per Cluster
                for clust_num in np.unique(labels):
                    ws = wb.create_sheet(f"RawData_Cluster{clust_num}")
                    clust_data = patterns[labels == clust_num]
                    dfc = pd.DataFrame(clust_data)
