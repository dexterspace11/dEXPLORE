...
def summarize_feature_series(feature_series):
    ...
    if len(unique_levels) == 1:
        ...
        return f"ranged from {unique_levels[0]} to {unique_levels[-1]}"
...
if uploaded_file:
    ...
    if len(selected) >= 2:
        ...
        if len(patterns) > 0:
            ...
            st.markdown(
                "The dendrogram above shows the hierarchical clustering structure. "
                "Shorter vertical lines imply clusters that are closer or more similar. "
            ...
            # Narrative Descriptions & Summary
            ...
            # Export to Excel with raw cluster data
            if st.button("Export Analysis to Excel"):
                ...
                wb.save(export_path)
                st.success(f"Exported analysis to Excel: {export_path}")

        else:
            st.warning("No patterns were learned. Try increasing the data size or adjusting parameters.")
    else:
