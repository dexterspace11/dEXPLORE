...
if uploaded_file:
    ...
    if len(selected) >= 2:
        ...
        if len(patterns) > 0:
            ...
            cluster_descriptions = generate_cluster_descriptions(centroids, selected * window_size)
            comp_summary = generate_comparative_summary(centroids, selected * window_size)
            ...
            if st.button("Export Analysis to Excel"):
                ...
                df_centroids = pd.DataFrame(centroids, columns=[f"{col}_t{t}" for t in range(window_size) for col in selected])
                for r in dataframe_to_rows(df_centroids, index=False, header=True):
                    ws4.append(r)
                for i in range(k):
                    cluster_data = patterns[labels == i]
                    expanded_columns = [f"{col}_t{t}" for t in range(window_size) for col in selected]
                    df_cluster = pd.DataFrame(cluster_data, columns=expanded_columns)
